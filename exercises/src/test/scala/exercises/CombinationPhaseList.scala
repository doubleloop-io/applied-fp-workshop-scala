...
class CombinationPhaseList extends munit.FunSuite {
  ...
  test("all valid - individual validation results".ignore) {
  ...
  test("some invalid - individual validation results".ignore) {
  ...
  test("all valid - one omni validation result".ignore) {
    val values = List("1", "10", "100")
  ...
  test("some invalid - one omni validation result".ignore) {
